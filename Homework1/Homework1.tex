...
A must then be a knave. As a result, B must be a knight, as he is telling the truth in this instance. Note that when A is a knave and B is a knight, both of them satisfy the requirements of the problem, as A is not telling the truth (as both of them are not knights) and B is telling the truth (as A is a knave).
\end{homeworkProblem}
\begin{homeworkProblem}
...
\item \((p{\lor}q){\rightarrow}{\neg}p{\equiv}{\neg}(p{\lor}q){\lor}{\neg}p{\equiv}({\neg}p{\land}{\neg}q){\lor}{\neg}p{\equiv}\)
\framebox[1.25\width]{\({\neg}p\)}
\item \((p{\rightarrow}{\neg}q){\rightarrow}({\neg}p{\rightarrow}q){\equiv}({\neg}p{\lor}{\neg}q){\rightarrow}(p{\lor}q){\equiv}{\neg}({\neg}p{\lor}{\neg}q){\lor}(p{\lor}q){\equiv}(p{\land}q){\lor}(p{\lor}q){\equiv}((p{\land}q){\lor}p){\lor}q{\equiv}\)
\framebox[1.1\width]{\((p{\lor}q)\)}
\end{enumerate}
\end{homeworkProblem}
\begin{homeworkProblem}
...
\begin{align*}
	...
	&{\equiv}(p{\lor}\textbf{F}){\lor}(p{\land}q) && \text{(Negation Law)} \\
	&{\equiv}p{\lor}(p{\land}q) && \text{(Identity Law)} \\
...
I also want to show a ``slick'' way to do this (as opposed to the method shown in class):\\
\begin{align*}
(p{\rightarrow}q){\land}(p{\rightarrow}{\neg}q){\land}({\neg}p{\rightarrow}q){\land}({\neg}p{\rightarrow}{\neg}q)
&{\equiv}(p{\rightarrow}q){\land}(p{\rightarrow}{\neg}q){\land}({\neg}p{\rightarrow}q){\land}(q{\rightarrow}p)
&& {\text {(${\neg}p{\rightarrow}{\neg}q{\equiv}q{\rightarrow}p$ By Contrapositive)}}\\
&{\equiv}(p{\rightarrow}q){\land}(q{\rightarrow}p){\land}(p{\rightarrow}{\neg}q){\land}({\neg}p{\rightarrow}q)
&& {\text {(Commutative Property)}}\\
&{\equiv}(p{\rightarrow}q){\land}(q{\rightarrow}p){\land}(p{\rightarrow}{\neg}q){\land}({\neg}q{\rightarrow}p)
&& {\text {(${\neg}p{\rightarrow}q{\equiv}{\neg}q{\rightarrow}p$ By Contrapositive)}}\\
&{\equiv}(p{\iff}q){\land}(p{\rightarrow}{\neg}q){\land}({\neg}q{\rightarrow}p)
&& {\text {(Page 29, Table 8)}}\\
&{\equiv}(p{\iff}q){\land}(p{\iff}{\neg}q)
&& {\text {(Page 29, Table 8)}}
\end{align*}
The last statement is a contradiction, as for the statement \(a{\iff}b\) to be true, then \(a\) and \(b\) must have the same value. However, for the last statement to be true, \(p\) must have the same value as both \(q\) and \({\neg}q\) which cannot be true, and thus the proposition is unsatisfiable.
\\
\textbf{Part C}\\
\begin{displaymath}
...
\end{array}
\end{displaymath}
Since the table is false for all values in the last (highlighted) column, \((p{\iff}q){\land}({\neg}p{\iff}q)\) is unsatisfiable. This can also be justified using the reasoning found in Part B, where \((p{\iff}q){\land}(p{\iff}{\neg}q)\) means that \(p\) must have the same value as both \(q\) and \({\neg}q\) which cannot be true, and thus the proposition is unsatisfiable.
\end{homeworkProblem}
\end{document}